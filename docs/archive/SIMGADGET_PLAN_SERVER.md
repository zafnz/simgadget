...
| 17 | the handshake instructions are rewritten: the coordinate space and `ui_view` move from last to third and fourth, the paragraphs a response explains by itself are cut back, "do not derive tap coordinates from a screenshot" narrows to a *saved screenshot file* and names ui_view's image as point space, and a timed-out `start_simulator` says it may still be booting. They are no longer diffed against the baseline: `tools.test.mts` pins them character for character, and `mcp.test.mts` proves the built server sends that string | clients truncate them, and one was observed cutting at ~2100 characters, which dropped exactly the last two. An agent that never read them drove a session off `screenshot` and scaled every coordinate by hand. The order is now what an agent cannot learn any other way first; a refused `ui_tap` explains itself in the refusal. The narrowed clause was written about the `screenshot` tool's file and was harmless while truncated away — surfaced, it forbids the one thing ui_view is best at, one sentence after recommending it |
| 18 | `ui_view` and `screenshot` each name the other, and say which one hands back a point-space image and which writes a pixel raster to a file | a harness that lists tools by name and charges for each schema fetch makes the description of the tool an agent *does* fetch the only place the fork is visible. `Takes a screenshot of the iOS Simulator` gave it no reason to look further |
